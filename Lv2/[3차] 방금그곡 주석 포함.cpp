...
string solution(string m, vector<string> musicinfos)
{
        ...
        for (int i = 0; i < mi.size(); ++i) {
                ...
                /*int melodyIdx = 0;
		bool change_answerIdx = false;
		for (int j = 0; j < time; ++j) {
			int mIdx = 0;
			while (m[mIdx] == mi[i].melody[melodyIdx]) {
				if (mIdx == m.size() - 1) { // 마지막 부분이라면
					if (answerIdx == -1)
						answerIdx = i;
					else
						answerIdx = ((mi[answerIdx].endTime - mi[answerIdx].startTime) >= (mi[i].endTime - mi[i].startTime) ? answerIdx : i);
...
/*string solution(string m, vector<string> musicinfos) {
	...
	for (int i = 0; i < infoSize; ++i) {
		...
		tmp.melody = musicinfos[i].substr(j + 1);
	...
	for (int i = 0; i < mi.size(); ++i) {
		...
		for (int j = 0; j < time; ++j) {
			int mIdx = 0;
			while (m[mIdx] == mi[i].melody[melodyIdx]) {
				if (mIdx + 1 < m.size() && m[mIdx] == '#') {
					if (m[mIdx + 1] == mi[i].melody[(melodyIdx + 1) % mi[i].melody.size()]) {
						++mIdx;
						++melodyIdx;
					}
					else {
						melodyIdx = (melodyIdx + 2) % mi[i].melody.size();
						break;
					}
				}
				if (mIdx == m.size() - 1) { // 마지막 부분이라면
					if (melodyIdx + 1 < mi[i].melody.size() && mi[i].melody[melodyIdx + 1] == '#') {    // fail case
						melodyIdx = (melodyIdx + 2) % mi[i].melody.size();
						break;
					}
					mi[i].ans = true;
			...
			if (melodyIdx + 1 < mi[i].melody.size() && mi[i].melody[melodyIdx + 1] == '#') {
				melodyIdx += 2;
				melodyIdx %= mi[i].melody.size();
			}
			else {
